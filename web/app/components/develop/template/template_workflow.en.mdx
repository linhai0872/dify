...
--data-raw '{
    ...
    ### Blocking Mode
    Blocking mode can return either a completed workflow result or a paused workflow result.

    When execution pauses for Human-in-the-Loop, the response still includes `workflow_run_id` and `task_id`, but `data.status` becomes `paused`.
    The paused payload also includes `paused_nodes` and `reasons`. For `human_input_required`, each reason contains the `form_id` and its `expiration_time`.
    ...
    Streaming mode can also pause for Human-in-the-Loop. In that case, the SSE stream emits `human_input_required` first and then `workflow_paused`.
...
<Row>
  <Col>
    ...
    - `continue_on_pause` (bool) Optional, set to `true` to keep the stream open across `workflow_paused` events until `workflow_finished`.

    ### Response
    Server-Sent Events stream (`text/event-stream`).
    Typical events include `workflow_paused`, `node_started`, `node_finished`, `human_input_form_filled`, `human_input_form_timeout`, and `workflow_finished`.
    If the workflow has already finished when you call this endpoint, the server returns a single finished event immediately.
  </Col>
  <Col sticky>
    <CodeGroup
      title="Request"
...
--header 'Authorization: Bearer {api_key}'`}
  ...
  url='/custom/remote-files/<url>'
  method='GET'
  title='Get Remote File Info'
  name='#custom-remote-file-info'
/>
<Row>
  <Col>
    Get metadata of a remote file, including file type and size. This endpoint accesses remote URLs through SSRF protection proxy.

    <i>This is a custom extension API.</i>

    ### Path Parameters
    - `url` (string) Required
      The URL of the remote file (URL encoded).

    ### Response
    - `file_type` (string) File MIME type
    - `file_length` (int) File size in bytes, -1 if unavailable

    ### Errors
    - 400, `remote_file_upload_error`, Failed to fetch remote file
  ...
  <Col sticky>
    ...
    <CodeGroup
      title="Request"
      tag="GET"
      label="/custom/remote-files/<url>"
      targetCode={`curl -X GET '${props.appDetail.api_base_url}/custom/remote-files/https%3A%2F%2Fexample.com%2Ffile.pdf' \\
--header 'Authorization: Bearer {api_key}'`}
    />
...
<Heading
  url='/custom/remote-files/upload'
  method='POST'
  title='Upload Remote File'
  name='#custom-remote-file-upload'
/>
<Row>
  <Col>
    Download a file from a remote URL and upload it to Dify storage. This endpoint downloads files through SSRF protection proxy.
    ...
    <Properties>
      <Property name='url' type='string' key='url'>
        The URL of the remote file (required).
      </Property>
      <Property name='user' type='string' key='user'>
        User identifier, used to define the identity of the end-user, must be consistent with the user passed in the send message API.
      </Property>
    </Properties>

    ### Response
    After successful upload, the server returns the file ID and signed URL.
    - `id` (uuid) File ID
    - `name` (string) File name
    - `size` (int) File size in bytes
    - `extension` (string) File extension
    - `url` (string) Signed access URL
    - `mime_type` (string) File MIME type
  ...
  <Col sticky>
    ...
    <CodeGroup
      title="Request"
      tag="POST"
      label="/custom/remote-files/upload"
      targetCode={`curl -X POST '${props.appDetail.api_base_url}/custom/remote-files/upload' \\
--header 'Authorization: Bearer {api_key}' \\
--header 'Content-Type: application/json' \\
--data-raw '{
...
<Heading
  ...
  method='GET'
  title='Get Execution by Trace ID'
  name='#custom-trace-search'
/>
<Row>
  <Col>
    Get complete execution details by external trace ID, including workflow node executions.

    <i>This is a custom extension API.</i>

    The trace ID can be provided via:
    - Request header `X-Trace-Id`
    - Query parameter `trace_id`
    - Request body `inputs.dify_trace_id`

    ### Path Parameters
    - `app_id` (uuid) Required - Application ID
    - `trace_id` (string) Required - External trace ID

    ### Response
    - `type` (string) Application type: workflow
    - `workflow_run` (object) Workflow run details
      - `id` (uuid) Run ID
      ...
      - `finished_at` (string) Finished timestamp
    - `node_executions` (array) Node execution list

    ### Errors
    - 401, Invalid API Token
    - 404, No execution found for trace ID
  ...
  <Col sticky>
    ...
    <CodeGroup
      ...
      targetCode={`# 1. Call workflow with trace_id
curl -X POST '${props.appDetail.api_base_url}/workflows/run' \\
--header 'Authorization: Bearer {api_key}' \\
--header 'X-Trace-Id: order-12345' \\
--header 'Content-Type: application/json' \\
--data-raw '{
...
# 2. Query execution by trace_id
curl -X GET '${props.appDetail.api_base_url}/custom/apps/${props.appDetail.id}/trace/order-12345' \\
--header 'Authorization: Bearer {api_key}'`}
    />
...
<Row>
  <Col>
    ...
    <Properties>
      ...
      <Property name='keyword_scope' type='string' key='keyword_scope'>
        [Custom] Search scope: all (default), inputs, outputs, session_id, run_id, trace_id
      </Property>
